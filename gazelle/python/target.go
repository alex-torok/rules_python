...
func newTargetBuilder(kind, name, pythonProjectRoot, bzlPackage string, siblingSrcs *treeset.Set) *targetBuilder {
	return &targetBuilder{
		...
		pytestFixtures:    treeset.NewWith(pytestFixtureUseCompartor),
	}
}
...
// addVisibility adds visibility labels to the target.
func (t *targetBuilder) addPytestFixtures(fixtures *treeset.Set) *targetBuilder {
	it := fixtures.Iterator()
	for it.Next() {
		t.pytestFixtures.Add(it.Value().(pytestFixtureUse))
	}
	return t
}
...
func (t *targetBuilder) build() *rule.Rule {
	...
	r.SetPrivateAttr(config.GazelleImportsKey, &genResultImport{
		moduleDeps:     t.deps,
		pytestFixtures: t.pytestFixtures,
	})

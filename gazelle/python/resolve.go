...
func (py *Resolver) Resolve(
	...
	if genResultImportRaw != nil {
		cfgs := c.Exts[languageName].(pythonconfig.Configs)
		cfg := cfgs[from.Pkg]
		pythonProjectRoot := cfg.PythonProjectRoot()
		genResImport := genResultImportRaw.(*genResultImport)
		explainDependency := os.Getenv("EXPLAIN_DEPENDENCY")

		resolvePytestFixtures(genResImport.pytestFixtures, cfg, from, deps, explainDependency)

		modules := genResImport.moduleDeps
		it := modules.Iterator()
		hasFatalError := false
	MODULES_LOOP:
		for it.Next() {
...
func resolvePytestFixtures(pytestFixtures *treeset.Set, cfg *pythonconfig.Config, from label.Label, deps *treeset.Set, explainDependency string) {
	it := pytestFixtures.Iterator()
	for it.Next() {
		fixture := it.Value().(pytestFixtureUse)
		if dep, ok := cfg.FindPytestFixtureResolve(fixture.Name); ok {
			l, err := label.Parse(dep)
			if err != nil {
				log.Printf("ERROR: failed to validate dependencies for target %q: invalid pytest fixture resolve label %q: %v", from.String(), dep, err)
				os.Exit(1)
			}

			// Prevent circular dependencies from a target to itself.
			if l.Equal(from) {
				continue
			}
			deps.Add(l.Rel(from.Repo, from.Pkg).String())
			if explainDependency == dep {
				log.Printf("Explaining dependency (%s): "+
					"in the target %q, the file %q function %q uses pytest fixture %q at line %d,"+
					"which resolves using the \"gazelle:python_resolve_pytest_fixture\" directive.\n",
					explainDependency, from.String(), fixture.Filepath, fixture.FunctionName, fixture.Name, fixture.LineNumber)
			}
		}
	}
}
